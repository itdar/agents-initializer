...
> プロジェクトディレクトリをスキャンし、
> `AGENTS.md` + ナレッジ/スキル/ロールのコンテキストを自動生成することで、AIエージェントが即座に作業を開始できます。

```
ワンコマンド → プロジェクト分析 → AGENTS.md生成 → あらゆるAIツールで動作
```
...
> **トークン使用量に関する注意** — 初回セットアップ時、最上位モデルがプロジェクト全体を分析し、複数のファイル（AGENTS.md、.ai-agents/context/、.ai-agents/skills/、.ai-agents/roles/）を生成します。プロジェクトの規模に応じて数万トークンを消費する場合があります。これは一度きりのコストであり、以降のセッションでは事前構築されたコンテキストを読み込んで即座に開始します。

```bash
# 1. AIにHOW_TO_AGENTS.mdを読ませれば、あとは自動的に処理されます

# オプションA: 英語（推奨 — トークンコストが低く、AI性能が最適）
...
# 推奨: --model claude-opus-4-6（またはそれ以降）で最良の結果を得られます
# 推奨: --dangerously-skip-permissions で中断のない自律実行が可能です
...
AIコーディングツールは**セッションごとにプロジェクトをゼロから学習し直します**。

| 問題 | 結果 |
|---|---|
| チームの規約を知らない | コードスタイルの不整合 |
| 完全なAPIマップを知らない | 毎回コードベース全体を探索（コスト+20%） |
| 禁止事項を知らない | 本番DBへの直接アクセスなどのリスクのある操作 |
| サービス依存関係を知らない | 副作用の見落とし |
...
> ETH Zurich (2026.03): **推論可能なコンテンツを含めると、成功率が低下し、コストが+20%増加する**

```
含めるべき（推論不可能）         .ai-agents/context/（推論コストが高い）   除外すべき（推論コストが低い）
───────────────────────        ────────────────────────────────────      ────────────────────────
チームの規約                    完全なAPIマップ                           ディレクトリ構造
禁止事項                        データモデルの関係性                       単一ファイルの内容
PR/コミットのフォーマット         イベントpub/sub仕様                      公式フレームワークドキュメント
隠れた依存関係                   インフラストラクチャトポロジー              インポート関係
```
...
```
project-root/
├── AGENTS.md                          # PMエージェント（全体のオーケストレーション）
├── .ai-agents/
│   ├── context/                       # ナレッジファイル（セッション開始時に読み込み）
│   │   ├── domain-overview.md         #   ビジネスドメイン、ポリシー、制約事項
...
│   └── roles/                         # ロール定義（ロール固有のコンテキスト深度）
│       ├── pm.md                      #   プロジェクトマネージャー
│       ├── backend.md                 #   バックエンド開発者
│       ├── frontend.md                #   フロントエンド開発者
│       ├── sre.md                     #   SRE / インフラストラクチャ
...
深度3までディレクトリを探索し、ファイルパターンに基づいて自動分類します。
...
検出されたタイプに基づいて、**実際にコードを分析して** `.ai-agents/context/` のナレッジファイルを生成します。
...
適切なテンプレートを使用して、各ディレクトリ用のAGENTS.mdを生成します。
...
ベンダー固有の設定にブリッジを追加し、**すべてのAIツールが**生成されたAGENTS.mdを読み取れるようにします。
...
> **原則:** ブートストラップファイルは、既に使用中のベンダーに対してのみ生成されます。未使用ツールの設定ファイルは作成されません。

---

## ベンダー互換性

| ツール | AGENTS.mdの自動読み取り | ブートストラップ |
|---|---|---|
...
| **Aider** | はい | `.aider.conf.yml`に読み込みを追加 |
...
報告:     子 → 親（タスク完了後の変更サマリー）
調整:     ピア間の直接通信なし — 親を通じた間接的な調整
```
...
```
セッション開始:
  1. AGENTS.mdを読み込む（ほとんどのAIツールはこれを自動的に行います）
...
コードが変更された場合、`.ai-agents/context/`のファイルも対応して更新する必要があります。
...
> 更新を怠ると、次のセッションで**古いコンテキストのまま作業する**ことになります。
...
<p align="center">
  <sub>AIエージェントがプロジェクトを理解するまでの時間をゼロにします。</sub>
</p>
