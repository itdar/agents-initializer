...
**하나의 명령으로 AI 에이전트에게 즉각적인 프로젝트 이해를 부여합니다.**

프로젝트 스캔 → `AGENTS.md` + 지식/스킬/역할 컨텍스트 생성
→ 어떤 AI 도구든 매 세션마다 즉시 작업 시작.
...
이 저장소에는 이미 `AGENTS.md`와 `.ai-agents/` 파일이 포함되어 있습니다.
...
→ 에이전트가 AGENTS.md + .ai-agents/context/ 자동 로드
→ 즉시 작업 가능!
```
...
> **중요:** `setup.sh`와 `HOW_TO_AGENTS.md`를 **적용하려는 본인 프로젝트 디렉토리**에 복사한 후 실행하세요.
> 이 파일들은 대상 프로젝트의 구조를 분석하므로, 반드시 해당 프로젝트 안에 있어야 합니다.

```bash
# 1. 파일을 프로젝트에 복사 (혹은 마우스로 끌어다놓기)
cp setup.sh HOW_TO_AGENTS.md /path/to/your-project/
cd /path/to/your-project

# 2. 대화형 세팅 실행 (AI 도구 + 언어 선택 후 자동 생성)
./setup.sh
...
이게 전부입니다. `setup.sh`가 도구 감지, 언어 선택, 전체 생성을 자동으로 처리합니다.

<details>
<summary><b>수동 설정 (setup.sh 없이)</b></summary>

```bash
cd /path/to/your-project

# 옵션 A: 영어 (권장 — 토큰 비용 낮음, AI 성능 최적)
...
> **토큰 안내:** 초기 설정 시 전체 프로젝트를 분석하여 수만 토큰이 소비될 수 있습니다. 이는 일회성 비용이며, 이후 세션에서는 미리 구축된 컨텍스트를 즉시 로드합니다.
...
```bash
./ai-agency.sh                    # 대화형: 에이전트 + 도구 선택
./ai-agency.sh --tool claude      # Claude로 바로 실행
./ai-agency.sh --agent api        # 키워드로 에이전트 선택
./ai-agency.sh --multi            # tmux 병렬 에이전트
./ai-agency.sh --list             # 에이전트 목록 출력
```

전체 실행 샘플은 [빠른 시작](#샘플-세팅-후-에이전트-실행) 섹션을 참조하세요.
...
│  1. 초기 설정 (일회성)                                               │
│                                                                  │
│  ./setup.sh 실행 (또는 수동으로 AI에게 HOW_TO_AGENTS.md 읽기 지시)      │
│       │                                                          │
│       ▼                                                          │
│  AI가 프로젝트 구조를 분석                                            │
│       │                                                          │
...
| 파일 | 대상 | 목적 |
|---|---|---|
| `setup.sh` | 사람 | 원커맨드 대화형 세팅 (도구 + 언어 → 자동 생성) |
