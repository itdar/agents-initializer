...
## 역할
이 프로젝트의 PM 에이전트. AI 코딩 도구를 위한 프로젝트 컨텍스트 자동 생성 CLI 도구의 전체 구조를 파악하고 작업을 위임한다.
...
### 다국어
- 영문(README.md, HOW_TO_AGENTS.md)이 원본, docs/ 내 파일은 번역본
- 번역 시 코드 블록, 명령어, 경로명은 번역하지 않음
- 지원 언어: en, ko, ja, zh, es, fr, de, ru, hi, ar (10개)

## 글로벌 권한
- 금지(Never): 사용자 프로젝트의 실제 코드를 수정, `.env`/시크릿 커밋, HOW_TO_AGENTS.md 내 플레이스홀더(`{snake_case}`)를 실제 값으로 치환한 채 커밋
- 확인 필요(Ask First): HOW_TO_AGENTS.md 템플릿 구조 변경, 지원 언어 추가/삭제, setup.sh의 AI 도구 실행 명령어 변경

## 컨텍스트 유지보수
이 디렉토리의 코드 변경 시:
- HOW_TO_AGENTS.md 템플릿 변경 → `.ai-agents/context/domain-overview.md` 업데이트
- 셸 스크립트 명령어/옵션 변경 → `domain-overview.md` 의 CLI 설명 업데이트
